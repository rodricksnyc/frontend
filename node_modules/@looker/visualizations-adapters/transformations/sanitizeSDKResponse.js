import "core-js/modules/es.array.concat.js";
import omit from 'lodash/omit';
export var keysToRemove = ['defaults_version', 'hidden_fields', 'hide_legend', 'interpolation', 'label_density', 'label_type', 'label_type', 'legend_position', 'plot_size_by_field', 'point_style', 'series_cell_visualizations', 'series_colors', 'series_labels', 'series_point_styles', 'series_types', 'show_null_points', 'show_value_labels', 'show_view_names', 'show_x_axis_label', 'show_x_axis_ticks', 'show_y_axis_labels', 'show_y_axis_ticks', 'size_by_field', 'stacking', 'trellis', 'value_labels', 'x_axis_gridlines', 'x_axis_reversed', 'x_axis_scale', 'y_axes', 'y_axis_combined', 'y_axis_gridlines', 'y_axis_reversed', 'y_axis_scale_mode', 'y_axis_tick_density', 'y_axis_tick_density_custom'];
export var sanitizeSDKResponse = function sanitizeSDKResponse(_ref) {
  var config = _ref.config,
      data = _ref.data,
      fields = _ref.fields;
  var unusedVar = undefined;
  return {
    config: omit(config, [].concat(keysToRemove)),
    data: data,
    fields: fields
  };
};
//# sourceMappingURL=sanitizeSDKResponse.js.map