...
var _utils = require("../InputTime/utils");
...
const formatLabel = (format, hour, minute) => {
  const formattedHour = (0, _utils.formatTimeString)(cycleHourDisplay(format, hour));
  const formattedMinute = (0, _utils.formatTimeString)(minute);
...
const generateMinuteIntervals = interval => {
  ...
  return (0, _map3.default)(minutes, (_, index) => (0, _utils.formatTimeString)(index * interval));
};
...
const generateTimes = (format, interval) => {
  ...
  return (0, _reduce.default)(hours, (result, _, hour) => {
    ...
    const hourWithMinutes = (0, _map3.default)(minutes, minute => {
      ...
      const value = `${(0, _utils.formatTimeString)(hour)}:${minute}`;
      return {
        label,
        value
      };
...
const matchClosestMinute = (interval, timeCode) => {
  const minuteOptions = (0, _map3.default)(generateMinuteIntervals(interval), _utils.parseBase10Int);
  const now = new Date(Date.now());
  const currentMinute = timeCode ? (0, _utils.parseBase10Int)(timeCode.split(':')[1]) : now.getMinutes();
  const currentHour = timeCode ? (0, _utils.parseBase10Int)(timeCode.split(':')[0]) : now.getHours();
  ...
  const formattedHour = (0, _utils.formatTimeString)(currentHour);
  const formattedMinute = (0, _utils.formatTimeString)(roundedMinute);
  return `${formattedHour}:${formattedMinute}`;
};

const createOptionFromStringValue = (format, value) => {
  const _map = (0, _map3.default)(value.split(':'), _utils.parseBase10Int),
...
const createOptionFromLabel = (format, label) => {
  ...
  const value = `${(0, _utils.formatTimeString)(hr24)}:${(0, _utils.formatTimeString)(minute)}`;

  if ((0, _utils.isValidTime)(value)) {
    return {
...
const matchStringValueToOption = (options, format, value) => {
  if (value && (0, _utils.isValidTime)(value)) {
...
const InputTimeSelectLayout = (0, _react.forwardRef)((_ref, ref) => {
  ...
  (0, _components.useReadOnlyWarn)('InputTimeSelect', value, onChange);
  ...
  const throttledHandleChange = (0, _react.useMemo)(() => (0, _throttle.default)(newSelectedOption => {
    setSelectedOption(newSelectedOption);
    const newValue = newSelectedOption ? newSelectedOption.value : undefined;

    if ((0, _isFunction.default)(onChange) && (0, _utils.isValidTime)(newValue)) {
      onChange(newValue);
    }
  }, 50, {
    trailing: false
  ...
  const ariaProps = (0, _components.pickAriaAndValidationProps)(props);
  return _react.default.createElement(_components.Combobox, {
    className: className,
    ref: ref,
    onChange: throttledHandleChange,
    value: selectedOption
  }, _react.default.createElement(_components.ComboboxInput, _extends({
    ...
    autoFocus: autoFocus,
    id: id
  }, ariaProps)), _react.default.createElement(_components.ComboboxList, {
    persistSelection: true
  }, timeOptionsFocused.map((option, index) => _react.default.createElement(_components.ComboboxOption, _extends({}, option, {
    key: index
  })))));
});
