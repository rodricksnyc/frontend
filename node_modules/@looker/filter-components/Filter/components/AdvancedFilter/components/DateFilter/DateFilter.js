...
export var DateFilter = function DateFilter(_ref) {
  var item = _ref.item,
  ...
  var typeChange = function typeChange(value) {
    return onChange(item.id, sanitizeDate(_objectSpread(_objectSpread({}, item), {}, {
      type: value
    })));
  };

  var dateFilterOptions = useDateFilterOptions();
  var type = convertTypeToMatchesAdvancedOption(item);
  ...
  var options = useFilterOptions(dateFilterOptions, showMatchesAdvanced);

  var handleOnAdd = function handleOnAdd() {
    return onAdd(newDateItem(item), true);
  };

  var FilterComponent = dateFilterTypeToFilter(item.type);
