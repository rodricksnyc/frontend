"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.useUnitOptions = exports.useLocationFilterOptions = void 0;

var _reactI18next = require("react-i18next");

var _get_user_attribute_option = require("./get_user_attribute_option");

const useLocationFilterOptions = () => {
  const _useTranslation = (0, _reactI18next.useTranslation)('get_location_filter_options'),
        t = _useTranslation.t;

  const userAttributeOption = (0, _get_user_attribute_option.useUserAttributeOption)();
  return [{
    value: 'location',
    label: t('Location')
  }, {
    value: 'circle',
    label: t('Circle')
  }, {
    value: 'box',
    label: t('Box')
  }, {
    value: 'anyvalue',
    label: t('is anywhere')
  }, {
    value: 'null',
    label: t('is null')
  }, {
    value: 'notnull',
    label: t('is not null')
  }, userAttributeOption];
};

exports.useLocationFilterOptions = useLocationFilterOptions;

const useUnitOptions = () => {
  const _useTranslation2 = (0, _reactI18next.useTranslation)('get_location_filter_options'),
        t = _useTranslation2.t;

  return [{
    value: 'feet',
    label: t('feet')
  }, {
    value: 'kilometers',
    label: t('kilometers')
  }, {
    value: 'meters',
    label: t('meters')
  }, {
    value: 'miles',
    label: t('miles')
  }];
};

exports.useUnitOptions = useUnitOptions;
//# sourceMappingURL=get_location_filter_options.js.map