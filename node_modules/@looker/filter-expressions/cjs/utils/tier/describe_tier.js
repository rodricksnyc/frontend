...
const describeMultiValue = (values, field) => {
  if (values) {
    if (field !== null && field !== void 0 && field.parameter && field !== null && field !== void 0 && field.has_allowed_values) {
      const valueMap = (0, _keyBy.default)(field.enumerations, 'value');
      return values.map(value => {
        var _valueMap$escapedValu;

        const escapedValue = (0, _escape_parameter_value.escapeParameterValue)(value);
    ...
    return values.map(_add_quotes.addQuotes).join(' or ');
  }
...
const match = ({
  is,
  value
}, _, field) => `${(0, _describe_is_item.describeIsItem)(is)} ${value && value.length ? describeMultiValue(value, field) : 'any value'}`;
...
const describeTier = (item, filterType, field) => (0, _defaultTo.default)(filterToStringMap[item.type], () => '')(item, filterType, field);
