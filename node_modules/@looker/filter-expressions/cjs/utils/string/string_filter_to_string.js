...
const matchToString = ({
  value,
  is
}) => (0, _is_item_to_string.default)(is, '', '-') + value.map(_quote_filter.quoteFilter).map(escapeWithDoubleLastEscape).join(`,${(0, _is_item_to_string.default)(is, '', '-')}`);
...
const startWithToString = ({
  value,
  is
}) => multiValueToString(value.map(_escape_with_caret.escapeWithCaret).map(escapeWithoutDoubleLastEscape), token => `${(0, _is_item_to_string.default)(is, '', '-') + String(token)}%`);

const endsWithToString = ({
  value,
  is
}) => multiValueToString(value.map(_escape_with_caret.escapeWithCaret).map(escapeWithDoubleLastEscape), token => `${(0, _is_item_to_string.default)(is, '', '-')}%${String(token)}`);
