...
var matchToString = function matchToString(_ref) {
  var value = _ref.value,
      is = _ref.is;
  return isItemToString(is, '', '-') + value.map(quoteFilter).map(escapeWithDoubleLastEscape).join(",".concat(isItemToString(is, '', '-')));
};
...
var startWithToString = function startWithToString(_ref2) {
  var value = _ref2.value,
      is = _ref2.is;
  return multiValueToString(value.map(escapeWithCaret).map(escapeWithoutDoubleLastEscape), function (token) {
...
var endsWithToString = function endsWithToString(_ref3) {
  var value = _ref3.value,
      is = _ref3.is;
  return multiValueToString(value.map(escapeWithCaret).map(escapeWithDoubleLastEscape), function (token) {
